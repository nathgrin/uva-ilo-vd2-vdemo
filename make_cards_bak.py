...
def make_figures(loc,obj):
    ...
    fig,ax = _inifig()
    arraytxt =""
    for i,antw in enumerate(obj['antw']):
        print(chr(i+1))
        arraytxt += "\n%s) %s "%(chr(1),antw)
        
    thetext = """%s %s"""%(obj['vraag'],arraytxt)
    print(thetext)
...
def main():
    ...
    obj = {'lvl':0,'cat':"keer",'vraag':"Bereken $%s 2\cdot3$, kies uit:"%(lognl(2)),'antw':['3','2','4','8']}
